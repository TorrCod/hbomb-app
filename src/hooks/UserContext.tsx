...
type _UserStateAction =
  ...
  | {
      type: "updatecartitem";
      payload: { itemCount: number; item: OLSitems }[];
    };
...
export const useUserContext = createContext<_UserState_Content>(
  _UserState_Content_init
);

export const UserProvider = ({ children }: any) => {
  const [state, dispatch] = useReducer(userReducer, _UserState_init);

  const loadingDone = (payload: boolean) =>
    dispatch({ type: "loadingdone", payload: !payload });
  ...
  const updateOrderList = () => {
    ...
    dashboard
      .fetchOrderList()
      .then((orderList) =>
        dispatch({ type: "updateorderlist", payload: orderList })
      );
  };
...
class HbombCart {
  ...
  addItem(): void {
    const isExist = this.cartItemList.filter(
      (e) => e.item.itemId === this.item.itemId
    );
  ...
  updateCount(payload: number): void {
    const cartItem = this.cartItemList.filter(
      (e) => e.item.itemId === this.item.itemId
    )[0];
  ...
  deleteCartItem(): void {
    const newCartItemList = this.cartItemList.filter(
      (e) => e.item.itemId !== this.item.itemId
    );
...
class Dashboard {
  ...
  async fetchOrderList(): Promise<Orders[]> {
    ...
    try {
      ...
      orderList = Object.values(response);
    } catch (e) {
      console.log(e);
    }
